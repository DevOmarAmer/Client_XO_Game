...
private void goBack() {
    ...
    // PRIORITY 4: Handle offline mode - Navigate based on game mode
    System.out.println("DEBUG: Navigating to appropriate screen (offline mode)");
    
    // Check if user was playing against computer
    if (mode == GameMode.HUMAN_VS_COMPUTER_MODE) {
        System.out.println("DEBUG: Computer mode detected - navigating to LEVEL_SELECTION");
        GameSession.clearSession(); // Clear scores and session data
